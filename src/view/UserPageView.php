...
class UserPageView{
	...
	// returnerar de användare som ska ingå i en ny grupp. 
	...
	// returnerar de tänkta gruppnamnet och antal användare till ny grupp. 
	...
	// returnerar html för användarsidan. 
	...
	// returnerar html för användarsidan när användaren har valt att skapa ny grupp med medlemmar. 
			...
			<form method='post' action='?action=" .NavigationView::$actionUserPageSelect. "'>
				<input type='hidden' name='" .self::$groupname."' value='$groupName'>
			 	<input type='hidden' name='" .self::$numberOfUsers."' value='$numberOfUsers'>";
					...
					// skapar fält beroende på hur många medlemmar användaren vill ha i den nya gruppen.
					for($i=1; $i<=$numberOfUsers; $i++){
						$html .="	
	...
	// Skapar html för de grupper som finns registrerade hos användaren. 
			...
			<div class = 'groupDiv'>
				<form method='post' action='?action=" .NavigationView::$actionGroup. "&amp;".self::$groupname."=" .urlencode($value)."'>
					...
					for($i = 0; $i < count($members); $i++){
						...
						// skriver ut medlemmarna som finns i gruppen (utan användaren namn). 
